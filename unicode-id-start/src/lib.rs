...
pub fn unicode_identifier_class(cp: char) -> UnicodeIdentifierClass {
    ...
    let mut bits = IDENTIFIER_OTHER;
    if unicode_id_start::is_id_start(cp) {
        bits |= IDENTIFIER_START;
    }
    if unicode_id_start::is_id_continue(cp) {
        bits |= IDENTIFIER_CONTINUE;
    }

    UnicodeIdentifierClass(bits)
}
...
pub fn str_is_identifier(s: &str) -> bool {
    ...
    if !unicode_identifier_class(
        s.chars().next().expect("len was checked to be > 0"),
    )
    .is_start()
    {
        return false;
